...
game: libserver.so libbase.so prepareDir
	$(CC) $(CFLAGS) $(GAME_FILES) $(GAME_LIBS) $(LIB_DIRS) $(GAME_INCLUDE) -o $(BIN_DIR)/$@
...
libserver.so: libbase.so prepareDir
	$(CC) $(CFLAGS) $(SERVER_FILES) $(SERVER_LIBS) $(LIB_DIRS) $(INCLUDE_DIRS) -shared -fPIC -o $(BIN_DIR)/$@
